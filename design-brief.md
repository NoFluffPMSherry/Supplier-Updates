...
**Coexisting with "Modify Part Number" (15 Jul 2026):** Flagged from a real screenshot — clicking a priced grid cell today opens the existing "Modify Part Number" pricing flow (method + markup %). The photo thumbnail can't sit inside that same click zone or it'll compete with a piece of core, high-frequency functionality. Resolved by making them two physically separate hit targets on the same cell: the price/markup/ETD block keeps its full click → Modify (unchanged), and the photo indicator moved to a small badge in the cell's top-right corner with its own click → photo lightbox. Neither can trigger the other. Worth a heads-up to whoever owns Modify Part Number (Mat Roggenkamp / IRO side) since this cell is jointly "owned" by two initiatives now.
